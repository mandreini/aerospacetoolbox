...
def _flowinput(flow):
    ...
    if len(flow) != 1:
        raise AerotbxValueError("Function needs exactly one flow input.")

    # pop the flow variable and type
    mtype, flow = flow.popitem()

    if not sp.isreal(gamma).all() or not sp.isreal(flow).all():
        raise AerotbxValueError("Flow input variables must be real numbers.")
    ...
    if (gamma <= 1).any():
        raise AerotbxValueError("Specific heat ratio inputs must be real numbers "
                                "greater than 1.")

    # if both inputs are non-scalar, they should be equal in shape
    if gamma.size > 1 and flow.size > 1 and gamma.shape != flow.shape:
        raise AerotbxValueError("Inputs must be same shape or at least one input "
                                "must be scalar.")
...
def flowisentropic(**flow):
    ...
    gamma, flow, mtype, itype = _flowinput(flow)  # calculate gamma-ratios for use in the equations
    a = (gamma + 1) / 2
    b = (gamma - 1) / 2
    c = a / (gamma - 1)
    ...
    if sp.isreal(flow).all():  # flow must be a real number check (previously just checked for M
        if mtype in ["mach", "m"]:
            if (flow < 0).any() or not sp.isreal(flow).all():
                raise AerotbxValueError("Mach number inputs must be real numbers "
                                        "greater than or equal to 0.")
            M = flow

        elif mtype in ["temp", "t"]:
            if (flow < 0).any() or (flow > 1).any():
                raise AerotbxValueError("Temperature ratio inputs must be real numbers "
                                        "0 <= T <= 1.")
            M[flow == 0] = sp.inf
            ...
            if (flow < 0).any() or (flow > 1).any():
                raise AerotbxValueError("Pressure ratio inputs must be real numbers "
                                        "0 <= P <= 1.")
            ...
            if (flow < 0).any() or (flow > 1).any():
                raise AerotbxValueError("Density ratio inputs must be real numbers "
                                        "0 <= rho <= 1.")
            ...
            if (flow < 1).any():
                raise AerotbxValueError("Area ratio inputs must be real numbers greater "
                                        "than or equal to 1.")

            M[:] = 0.2 if mtype == "sub" else 1.8
            for _ in range(_AETB_iternum):
                K = M ** 2
            ...
            raise TypeError("Keyword input must be an acceptable string to "
                            "select input parameter.")
    else:
        raise AerotbxValueError("Flow input must be real numbers.")
    ...
    if sp.isreal(flow).all():
        if mtype in ["mach", "m1", "m"]:
            ...
            if (flow < lowerbound).any() or (flow > 1).any():
                raise AerotbxValueError("Mach number downstream inputs must be real "
            ...
            if (flow < 0).any() or (flow > 1).any():
                raise AerotbxValueError("Total pressure ratio inputs must be real "
                                        "numbers 0 <= P0 <= 1.")
            ...
            if (flow < lowerbound).any():
                raise AerotbxValueError("Rayleigh-Pitot ratio inputs must be real "
...
def flowprandtlmeyer(**flow):
    ...
    if mtype in ["mach", "m"]:
        if (flow < 1).any():
            raise AerotbxValueError("Mach number inputs must be real numbers greater"
                                    " than or equal to 1.")
        M = flow

    elif mtype in ["mu", "machangle"]:
        if (flow < 0).any() or (flow > 90).any():
            raise AerotbxValueError("Mach angle inputs must be real numbers "
                                    "0 <= M <= 90.")
        M = 1 / sp.sin(sp.deg2rad(flow))

    elif mtype in ["nu", "pm", "pmangle"]:
        if (flow < 0).any() or (flow > 90 * ((1 / l) - 1)).any():
            raise AerotbxValueError("Prandtl-Meyer angle inputs must be real"
